...
## Difference from /esf:add-phase

- **`/esf:add-phase --after N`**: Inserts at position N+1 (after phase N)
- **`/esf:insert-phase --at N`**: Inserts at position N (pushing current N to N+1)
...
```bash
/esf:insert-phase [name] --at N [--description "text"] [--force]
```
...
```bash
/esf:insert-phase urgent-security-fix --at 2 --description "Critical security vulnerability fixes"
```
...
```bash
/esf:insert-phase project-setup --at 1 --description "Initial setup and configuration"
```
...
For inserting at the end, use `/esf:add-phase` instead (simpler, no renumbering):

```bash
# Don't do this:
/esf:insert-phase new-feature --at 6  # (if 5 phases exist)

# Do this instead:
/esf:add-phase new-feature
```
...
Options:
   ...
   2. Remove existing phase: /esf:remove-phase 2
```
...
/esf:add-phase urgent-fix --after 1
# Creates phase 02-urgent-fix
# Renumbers: 02→03, 03→04

/esf:insert-phase urgent-fix --at 2
# Creates phase 02-urgent-fix
# Renumbers: 02→03, 03→04
# (Same result in this case)

/esf:insert-phase urgent-fix --at 1
...
- `/esf:add-phase [name] --after N` - Simpler alternative (inserts after N)
- `/esf:remove-phase N` - Remove a phase
- `/esf:renumber-phases` - Fix numbering manually if needed
