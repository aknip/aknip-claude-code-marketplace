...
2. **Abort if config.json missing:**
   ```bash
   [ ! -f .planning/config.json ] && echo "ERROR: No config found. Run /esf:new-project first." && exit 1
   ```

3. **Abort if use cases already exist:**
   ```bash
   if [ -f .planning/use-cases/index.md ]; then
       echo "ERROR: Use cases already extracted. Use /esf:progress to check status."
       exit 1
   fi
   ```
