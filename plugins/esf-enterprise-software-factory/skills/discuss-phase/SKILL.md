...
**After this command:** Run `/esf:plan-phase N` to create execution plans.
...
if [ -z "$PHASE_ARG" ]; then
  echo "ERROR: Phase number required. Usage: /esf:discuss-phase <N>"
  exit 1
fi
```
...
`/esf:plan-phase ${PHASE_ARG}` — create execution plans (will use this context)
...
- [ ] User knows next step is `/esf:plan-phase N`
