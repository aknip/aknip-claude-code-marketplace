...
- Before starting phase execution (`/esf:execute-phase`)
- Returning to phase after break
- Reviewing decisions made during `/esf:discuss-phase`
- Handoff to another developer
- Resolving questions during implementation

## Prerequisites

- Phase has CONTEXT.md file (created by `/esf:discuss-phase`)
- Phase has been discussed/planned

## Usage

```bash
/esf:list-phase-assumptions [N]
```
...
Recommendations:
   1. Discuss phase first: /esf:discuss-phase 3
   2. Or plan without discussion: /esf:plan-phase 3 --skip-discussion

Note: CONTEXT files are created during /esf:discuss-phase to capture
...
Recommendations:
   ...
   2. Or re-run discussion: /esf:discuss-phase 2
...
```bash
/esf:list-phase-assumptions 2 --condensed
```
...
Full view: /esf:list-phase-assumptions 2
```

## Invalid Phase

```bash
/esf:list-phase-assumptions 99
```
...
List assumptions:
   /esf:list-phase-assumptions [1-3]
```

## Export to File

```bash
/esf:list-phase-assumptions 2 --output assumptions.txt
```
...
```bash
/esf:list-phase-assumptions 2 3
```
...
- `/esf:discuss-phase N` - Create CONTEXT file with assumptions
- `/esf:plan-phase N` - Planning uses these assumptions
- `/esf:execute-phase N` - Execution follows these assumptions
