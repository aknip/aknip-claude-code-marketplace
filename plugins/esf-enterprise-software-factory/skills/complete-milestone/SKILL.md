...
```bash
/esf:complete-milestone [--version X.Y.Z] [--tag-only] [--no-archive]
```
...
If any checks fail, the command will report issues and suggest running `/esf:audit-milestone` for details.
...
Run `/esf:new-milestone` to start next version.
```
...
- `/esf:audit-milestone` - Check milestone readiness before completing
- `/esf:new-milestone` - Start next milestone after completion
- `/esf:progress` - View current milestone progress
