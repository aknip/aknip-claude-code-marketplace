...
**After this command:** Phase complete if verification passes, or run `/esf:plan-phase N --gaps` to close gaps.
...
@./use-case-driven/workflows/execute-phase.md
...
PHASE_ARG="${1:-}"
if [ -z "$PHASE_ARG" ]; then
  echo "ERROR: Phase number required. Usage: /esf:execute-phase N"
  exit 1
fi
```
...
if [ "$PLAN_COUNT" -eq 0 ]; then
  echo "ERROR: No PLAN.md files found. Run /esf:plan-phase ${PHASE_ARG} first."
  exit 1
fi
```
...
/esf:plan-phase [N+1] — plan next phase
...
/esf:plan-phase ${PHASE_ARG} --gaps — create gap closure plans
