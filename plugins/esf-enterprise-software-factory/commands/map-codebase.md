...
- **Before** `/esf:new-project` on existing codebase
...
```bash
/esf:map-codebase [--output FILE] [--quick] [--detailed]
```
...
Note: Quick scan provides basic info.
      For comprehensive analysis, run: /esf:map-codebase --detailed
```

## Detailed Scan Mode

```bash
/esf:map-codebase --detailed
```
...
```bash
/esf:map-codebase --output docs/ARCHITECTURE.md
```
...
Options:
   ...
   2. Skip mapping and start fresh: /esf:new-project
   3. Cancel
...
Recommendations:
   ...
   3. Focus on specific directory: cd src/feature && /esf:map-codebase
