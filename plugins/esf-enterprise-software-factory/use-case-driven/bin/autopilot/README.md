...
Terminal UI for autonomous sprint execution across **any** UC-driven project.


Der Node. js Autopilot ruft Claude 2x pro Sprint auf:

Sprint 2:
_ Claude Aufruf 1: plan-sprint-2.prompt-md (falls keine Pläne
existieren)
— Claude Aufruf 2: execute-sprint-2.prompt.md (führt ALLE Teilpläne aus

Ablauf im Detail
Schritt: 1
CLaude-Aufruf: plan-sprint-N.prompt.md
Was passiert: Erstellt 02-01-PLAN, 02-02-PLAN, 02-03-PLAN

Schritt: 2
CLaude-Aufruf: execute-sprint-N.prompt.md
Was passiert: Führt alle Pläne der Sprint aus (Sub-Sprint 1, dann Sub-Sprint 2, etc.)
...
- Sprint 2 mit 3 Plänen (02-01, 02-02, 02-03) = 1-2 Claude-Aufrufe
...
Claude pro Sprint aufgerufen, nicht pro Teilplan.
...
uc-autopilot --sprints "1,2,3,4"

# Explicit project directory
uc-autopilot --project-dir /path/to/project --sprints "1,2,3,4"
...
npx tsx src/index.tsx --project-dir /path/to/project --sprints "1,2,3"

# Or via bin script
node bin/autopilot.js --project-dir /path/to/project --sprints "1,2,3"
```
...
| `-p, --sprints <sprints>` | Comma-separated sprint numbers | "1" |
| `-c, --checkpoint-mode <mode>` | queue, pause, or skip | "queue" |
| `-r, --max-retries <n>` | Max retries per sprint | 3 |
...
```
.planning/
├── PROJECT-PLAN.md              # Sprint definitions (required)
├── PROJECT-STATUS.md                # Execution state (auto-created)
└── sprints/                 # Sprint directories (auto-created)

.claude/use-case-driven/
└── templates/prompts/      # Prompt templates (required)
    ├── plan-sprint-prompt.md
    ├── execute-sprint-prompt.md
...
│   ├── Header.tsx         # Project name + sprint counter
│   ├── SprintInfo.tsx      # Current sprint details
...
├── execution/
│   ├── execute-sprint.ts   # Single sprint execution
...
- **Progress bar**: Visual sprint completion tracking
- **Token tracking**: Real-time token/cost monitoring
- **Budget limits**: Automatic stop when budget exceeded
- **Git safety**: Commits orphaned files before sprint changes
...
npx tsx src/index.tsx --dry-run --project-dir /path/to/project --sprints "1"
```
...
### "PROJECT-PLAN.md not found"
Run `/esf:new-project` then `/esf:use-case-analysis` first to initialize the UC project structure.
